(function() {
   ...
   * { path, name, type, user, perms, mtime }
  ...
  /**
   * Sort the list of files provided in input.
   * If file is a string (error), it returns it.
   *
   * < files: object return by ls
   * < strategy: sorting strategy (name or date)
   *
   * > ls object list sorted
   */
  function sorted(files, strategy) {
    if (typeof files == 'string') {
      return files;
    }
    if (!files.length || files.length === 1) return files;

    var hasDoubleDot = files.some(f => f.name === '..');

    var filesToSort = files;
    if (hasDoubleDot) {
      var doubleDot = files[0];
      var filesToSort = files.slice(1);
    }

    switch (strategy) {
      case 'date':
        var sortFn = (f1, f2) => util.compareDates(f1.mtime, f2.mtime);
        break;
      case 'reverse-date':
        var sortFn = (f1, f2) => -util.compareDates(f1.mtime, f2.mtime);
        break;
      case 'reverse-name':
        var sortFn = (f1, f2) => -util.compareStrings(f1.name, f2.name);
        break;
      case 'name':
      default:
        var sortFn = (f1, f2) => util.compareStrings(f1.name, f2.name);
    ...
    var sorted = filesToSort.sort(sortFn)

    if (hasDoubleDot) {
      return [doubleDot, ...sorted];
    }
    return filesToSort.sort(sortFn)
  }
