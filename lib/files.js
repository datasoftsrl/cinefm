...
function perms(mode) {
  ...
  return mask
    .map((m, i) => mode & parseInt(m, 8)? rwx[i % 3] : '-')
    .join('');
}
...
function _onComplete(error, retCode, cmd, context) {
  ...
  // remove tmp file
  fsutils.removeSync(fileFrom, { symbolicLinks: false });

  const elapsed = util.dateDiff(Date.now(), timeStart);
  const logMessage = `${elapsed.str || '<ND>'} - ${sizeInMB}MB`;

  for (let i=0; i<srcPath.length; ++i) {
    const logPath = `${srcPath[i]} -> ${dstPath}`;
    ...
    if (callbackStat) {
      if (!error) {
        callbackStat(
          srcNoRoot[i],
          'copied',
          `<p>${srcNoRoot[i]} → ${dstNoRoot[i]}: copied</p>`,
          {
            path: logPath,
        ...
        callbackStat(
          srcNoRoot[i],
          'error',
          `<p>${srcNoRoot[i]} → ${dstNoRoot[i]}: error</p>`,
          {
            path: logPath,
...
function localCp(names, dst, rootSrc, rootDst, callbackProg, callbackStat) {
  ...
  const context = {
    ...
    callbackProg,
    callbackStat,
    srcPath: names.map(name => path.join(rootSrc, name)),
    srcNoRoot: names,
    dstNoRoot: names.map(name => path.join(dst, path.basename(name))),
    fileFrom: util.writeFilesList(names, `/tmp/files-to-copy_${Date.now()}`),
    timeStart: Date.now(),
    sizeInMB: 0,
    cwd: path.join(rootSrc, path.dirname(names[0])),
  }

  const rsyncCmd = new rsync()
    // archive (preserve link, perms, owner, group...)
    .flags('a')
    // recursive
    .flags('r')
    ...
    .set('info', 'progress2')
    .set('files-from', context.fileFrom)
    .source(context.cwd)
