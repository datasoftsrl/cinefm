...
/**
 * Write a list of filenames to a file at the provided path.
 * 
 * < names: an array of strings 
 * < dst: destination path of files 
 * 
 * > path of the written file, undefined otherwise
 */
function writeFilesList(names, dst) {
  const joinedNames = (names || [])
    .map(p => path.basename(p))
    .join('\n');

  if (joinedNames) {
    fs.writeFileSync(dst, joinedNames + '\n');
    return dst;
  }
}
